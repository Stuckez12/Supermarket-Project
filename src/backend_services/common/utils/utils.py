...
def user_proto_format(user):
    ...
    return user_login_pb2.UserData(
        uuid=user.uuid,
        email=user.email,
        password_last_changed_at=str(int(user.password_last_changed_at.timestamp())),
        first_name=user.first_name,
        last_name=user.last_name,
        gender=user.gender,
        date_of_birth=str(user.date_of_birth),
        created_at=str(int(user.created_at.timestamp())),
        updated_at=str(int(user.updated_at.timestamp())),
        last_login=str(int(user.last_login.timestamp())),
